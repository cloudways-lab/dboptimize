...
class WP_DbOptimization_revisions extends WP_DbOptimization {
	...
	public function preview($params) {
		...
		if (!empty($posts)) {
			foreach ($posts as $key => $post) {
				$posts[$key]['post_title'] = '' == $post['post_title'] ? '('.__('no title', 'wp-dboptimize').')' : $post['post_title'];
			}
		}
		...
		return array(
			'id_key' => 'ID',
			'columns' => array(
				'ID' => __('ID', 'wp-dboptimize'),
				'post_title' => __('Title', 'wp-dboptimize'),
				'post_date' => __('Date', 'wp-dboptimize'),
			),
			'offset' => $params['offset'],
			'limit' => $params['limit'],
			'total' => $total,
			'data' => $this->htmlentities_array($posts, array('ID')),
			'message' => $total > 0 ? '' : __('No post revisions found', 'wp-dboptimize'),
		);
	}
	...
	public function after_optimize() {
		$message = sprintf(_n('%s post revision deleted', '%s post revisions deleted', $this->processed_count, 'wp-dboptimize'), number_format_i18n($this->processed_count));
	...
	public function after_get_info() {
		if ($this->found_count > 0) {
			$message = sprintf(_n('%s post revision in your database', '%s post revisions in your database', $this->found_count, 'wp-dboptimize'), number_format_i18n($this->found_count));
		} else {
			$message = __('No post revisions found', 'wp-dboptimize');
		}
	...
	public function settings_label() {
		...
		if ('false' == $this->retention_enabled && 'true' == $this->revisions_retention_enabled) {
			return sprintf(__('Clean post revisions but keep at least %d revisions', 'wp-dboptimize'), $this->revisions_retention_count);
		}

		return __('Clean all post revisions', 'wp-dboptimize');
	}
