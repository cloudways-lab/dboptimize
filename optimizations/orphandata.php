...
class WP_DbOptimization_orphandata extends WP_DbOptimization {
	...
	public function after_optimize() {
		$message = sprintf(_n('%s orphaned relationship data deleted', '%s orphaned relationship data deleted', $this->processed_count, 'wp-dboptimize'), number_format_i18n($this->processed_count));
	...
	public function after_get_info() {
		if ($this->found_count > 0) {
			$message = sprintf(_n('%s orphaned relationship data in your database', '%s orphaned relationship data in your database', $this->found_count, 'wp-dboptimize'), number_format_i18n($this->found_count));
		} else {
			$message = __('No orphaned relationship data in your database', 'wp-dboptimize');
		}
	...
	public function settings_label() {
		return __('Clean orphaned relationship data', 'wp-dboptimize');
	}
}
