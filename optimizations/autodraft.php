...
class WP_DbOptimization_autodraft extends WP_DbOptimization {
	...
	public function preview($params) {
		...
		return array(
			...
			'message' => $total > 0 ? '' : __('No auto draft posts found', 'wp-dboptimize'),
		);
	}
	...
	public function after_optimize() {
		$info_message = sprintf(_n('%s auto draft deleted', '%s auto drafts deleted', $this->processed_count, 'wp-dboptimize'), number_format_i18n($this->processed_count));

		if ($this->is_multisite_mode()) {
			$info_message .= ' ' . sprintf(_n('across %s site', 'across %s sites', count($this->blogs_ids), 'wp-dboptimize'), count($this->blogs_ids));
		}
	...
	public function after_get_info() {
		if (0 != $this->found_count && null != $this->found_count) {
			...
			$message = __('No auto draft posts found', 'wp-dboptimize');
		}
	...
	public function get_auto_option_description() {
		return __('Remove auto-draft posts', 'wp-dboptimize');
	}
}
