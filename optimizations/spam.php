...
class WP_DbOptimization_spam extends WP_DbOptimization {
	...
	public function preview($params) {
		...
		return array(
			...
			'message' => $total > 0 ? '' : __('No spam or trashed comments found', 'wp-dboptimize'),
		);
	}
	...
	public function after_optimize() {
		$message = sprintf(_n('%s spam comment deleted', '%s spam comments deleted', $this->processed_spam_count, 'wp-dboptimize'), number_format_i18n($this->processed_spam_count));
		$message1 = sprintf(_n('%s comment removed from Trash', '%s comments removed from Trash', $this->processed_trash_count, 'wp-dboptimize'), number_format_i18n($this->processed_trash_count));
	...
	public function after_get_info() {
		...
		if ($this->found_spam_count > 0) {
			$message = sprintf(_n('%s spam comment found', '%s spam comments found', $this->found_spam_count, 'wp-dboptimize'), number_format_i18n($this->found_spam_count));
			...
			if (!WP_DbOptimize::is_premium()) {
				$message .= ' | <a id="wp-dboptimize-edit-comments-spam" href="'.admin_url('edit-comments.php?comment_status=spam').'">'.' '.__('Review', 'wp-dboptimize').'</a>';
			}

		} else {
			$message = __('No spam comments found', 'wp-dboptimize');
		}

		if ($this->found_trash_count > 0) {
			$message1 = sprintf(_n('%s trashed comment found', '%s trashed comments found', $this->found_trash_count, 'wp-dboptimize'), number_format_i18n($this->found_trash_count));

			// if current version is not premium and Preview feature not supported then
			// add to message Review link to comments page
			if (!WP_DbOptimize::is_premium()) {
				$message1 .= ' | <a id="wp-dboptimize-edit-comments-trash" href="'.admin_url('edit-comments.php?comment_status=trash').'">'.' '.__('Review', 'wp-dboptimize').'</a>';
			}
		} else {
			$message1 = __('No trashed comments found', 'wp-dboptimize');
		}
