...
class WP_DbOptimization_trash extends WP_DbOptimization {
	...
	public function preview($params) {
		...
		return array(
			...
			'message' => $total > 0 ? '' : __('No trashed posts found', 'wp-dboptimize'),
		);
	}
	...
	public function after_optimize() {
		$message = sprintf(_n('%s post removed from Trash', '%s posts removed from Trash', $this->processed_count, 'wp-dboptimize'), number_format_i18n($this->processed_count));
	...
	public function after_get_info() {
		if ($this->found_count > 0) {
			$message = sprintf(_n('%s trashed post in your database', '%s trashed posts in your database', $this->found_count, 'wp-dboptimize'), number_format_i18n($this->found_count));
		} else {
			$message = __('No trashed posts found', 'wp-dboptimize');
		}
	...
	public function get_auto_option_description() {
		return __('Remove trashed posts', 'wp-dboptimize');
	}
}
