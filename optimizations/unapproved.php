...
class WP_DbOptimization_unapproved extends WP_DbOptimization {
	...
	public function preview($params) {
		...
		if (!empty($comments)) {
			foreach ($comments as $key => $comment) {
				$comments[$key]['post_title'] = array(
					'text' => '' == $comment['post_title'] ? '('.__('no title', 'wp-dboptimize').')' : $comment['post_title'],
		...
		return array(
			...
			'columns' => array(
				...
				'post_title' => __('Post', 'wp-dboptimize'),
				'comment_date' => __('Date', 'wp-dboptimize'),
			),
			'offset' => $params['offset'],
			'limit' => $params['limit'],
			'total' => $total,
			'data' => $this->htmlentities_array($comments, array('comment_ID')),
			'message' => $total > 0 ? '' : __('No unapproved comments found', 'wp-dboptimize'),
		);
	}
	...
	public function after_optimize() {
		$message = sprintf(_n('%s unapproved comment deleted', '%s unapproved comments deleted', $this->processed_count, 'wp-dboptimize'), number_format_i18n($this->processed_count));
	...
	public function after_get_info() {
		if ($this->found_count) {
			$message = sprintf(_n('%s unapproved comment found', '%s unapproved comments found', $this->found_count, 'wp-dboptimize'), number_format_i18n($this->found_count));
		} else {
			$message = __('No unapproved comments found', 'wp-dboptimize');
		}
