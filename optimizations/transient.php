...
class WP_DbOptimization_transient extends WP_DbOptimization {
	...
	public function preview($params) {
		...
		if ('single' == $type) {
			$columns = array(
				'option_id' => __('ID', 'wp-dboptimize'),
				'option_name' => __('Name', 'wp-dboptimize'),
				'option_value' => __('Value', 'wp-dboptimize'),
			);
		} else {
			$columns = array(
				'meta_id' => __('ID', 'wp-dboptimize'),
				'meta_key' => __('Name', 'wp-dboptimize'),
				'meta_value' => __('Value', 'wp-dboptimize'),
			);
		}
		...
		return array(
			...
			'message' => $total > 0 ? '' : __('No transient options found', 'wp-dboptimize'),
		);
	}
	...
	public function after_optimize() {
		...
		$message = sprintf(_n('%s transient option deleted', '%s transient options deleted', $this->processed_count, 'wp-dboptimize'), number_format_i18n($this->processed_count));
		...
		if (is_multisite() && is_main_network()) {
			...
			$message2 = sprintf(_n('%s network-wide transient option deleted', '%s network-wide transient options deleted', $sitemeta_table_transients_deleted, 'wp-dboptimize'), number_format_i18n($sitemeta_table_transients_deleted));
	...
	public function after_get_info() {
		...
		if ($this->found_count_all > 0) {
			$message = sprintf(_n('%1$d of %2$d transient option expired', '%1$d of %2$d transient options expired', $this->found_count_all, 'wp-dboptimize'), number_format_i18n($this->found_count), number_format_i18n($this->found_count_all));
		} else {
			$message = __('No transient options found', 'wp-dboptimize');
		}

		if ($this->is_multisite_mode()) {
			$message .= ' ' . sprintf(_n('across %d site', 'across %d sites', count($this->blogs_ids), 'wp-dboptimize'), count($this->blogs_ids));
		}
		...
		if ($this->is_multisite_mode()) {
			if ($sitemeta_table_transients_all > 0) {
				$message2 = sprintf(_n('%1$d of %2$d network-wide transient option found', '%1$d of %2$d network-wide transient options found', $sitemeta_table_transients_all, 'wp-dboptimize'), number_format_i18n($sitemeta_table_transients), number_format_i18n($sitemeta_table_transients_all));
				$message2 = $this->get_preview_link($message2, array('data-type' => 'multisite'));
			} else {
				$message2 = __('No site-wide transient options found', 'wp-dboptimize');
			}
		...
		if ($this->found_count_all > 0 || ($sitemeta_table_transients + $sitemeta_table_transients_all > 0)) {
			...
			$this->register_output('<input id="remove_all_transients" name="remove_all_transients" type="checkbox" value="true" '.checked($remove_all_transients, 'true').'><label for="remove_all_transients" style="color: inherit;">'.__('Remove all transient options (not only expired)', 'wp-dboptimize').'</label>');
		}
	}
	...
	public function settings_label() {
		return __('Remove expired transient options', 'wp-dboptimize');
	}
