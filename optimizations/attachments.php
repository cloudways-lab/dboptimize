...
class WP_DbOptimization_attachments extends WP_DbOptimization {
	...
	public function after_optimize() {
		...
		$message = sprintf(_n('%s orphaned attachment deleted', '%s orphaned attachments deleted', $this->processed_count, 'wp-dboptimize'), number_format_i18n($this->processed_count));
	...
	public function after_get_info() {
		...
		if ($this->found_count) {
			$message = sprintf(_n('%s orphaned attachment found', '%s orphaned attachments found', $this->found_count, 'wp-dboptimize'), number_format_i18n($this->found_count));
		} else {
			$message = __('No orphaned attachments found', 'wp-dboptimize');
		}
	...
	public function settings_label() {
		return __('Remove orphaned attachments', 'wp-dboptimize');
	}
