...
if (!class_exists('Updraft_Notices_1_0')) require_once(WPODB_PLUGIN_MAIN_PATH.'includes/updraft-notices.php');

class WP_DbOptimize_Notices extends Updraft_Notices_1_0 {
	...
	public function is_backup_plugin_installed($product = null, $also_require_active = false) {
		...
		return apply_filters('wp_dboptimize_is_backup_plugin_installed', false);
	}
	...
	protected function translation_needed($plugin_base_dir = null, $product_name = null) {// phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
		return parent::translation_needed(WPODB_PLUGIN_MAIN_PATH, 'wp-optimize');
	}
	...
	protected function render_specified_notice($advert_information, $return_instead_of_echo = false, $position = 'top') {
		...
		$extract_variables = array_merge($advert_information, array('wp_dboptimize_notices' => $this));

		return WP_DbOptimize()->include_template('notices/'.$template_file, $return_instead_of_echo, $extract_variables);
	}
}
