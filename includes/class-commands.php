...
class WP_DbOptimize_Commands {
	...
	public function get_status_box_contents() {
		return WP_DbOptimize()->include_template('database/status-box-contents.php', true, array('optimize_db' => false));
	}
	
	public function get_optimizations_table() {
		return WP_DbOptimize()->include_template('database/optimizations-table.php', true);
	}
	...
	public function get_wp_optimize_contents() {
		$content = WP_DbOptimize()->include_template('database/optimize-table.php', true, array('optimize_db' => false, 'load_data' => WP_DbOptimize()->template_should_include_data()));
		if (WP_DbOptimize()->is_updraft_central_request()) {
	...
	public function get_table_information_contents() {
		$content = WP_DbOptimize()->include_template('database/tables.php', true, array('optimize_db' => false, 'load_data' => WP_DbOptimize()->template_should_include_data()));
	...
	public function get_settings_contents() {
		...
		$admin_settings .= WP_DbOptimize()->include_template('database/settings-general.php', true, array('optimize_db' => false));
		$admin_settings .= WP_DbOptimize()->include_template('database/settings-auto-cleanup.php', true, array('optimize_db' => false, 'show_innodb_option' => WP_DbOptimize()->template_should_include_data() && $this->optimizer->show_innodb_force_optimize()));
		$admin_settings .= WP_DbOptimize()->include_template('settings/settings-logging.php', true, array('optimize_db' => false));
		...
		$admin_settings .= '</form>';
		$admin_settings .= WP_DbOptimize()->include_template('settings/settings-trackback-and-comments.php', true, array('optimize_db' => false));
	...
	public function get_js_translation() {
		$translations = WP_DbOptimize()->wpo_js_translations();
		...
		if (is_callable(array(WP_DbOptimize(), 'get_loggers_classes_info'))) {
			$translations['loggers_classes_info'] = WP_DbOptimize()->get_loggers_classes_info();
		}
	...
	public function get_table_list($data = array()) {
		if (isset($data['refresh_plugin_json']) && filter_var($data['refresh_plugin_json'], FILTER_VALIDATE_BOOLEAN)) WP_DbOptimize()->get_db_info()->update_plugin_json();

		$size = $this->optimizer->get_current_db_size();
	
		return apply_filters('wpo_get_tables_data', array(
			'table_list' => WP_DbOptimize()->include_template('database/tables-body.php', true, array('optimize_db' => false)),
			'total_size' => $size[0]
		));
	}
	...
	/**
	 * Do action wp_dboptimize_after_optimizations
	...
	public function optimizations_done() {
		...
		do_action('wp_dboptimize_after_optimizations');
	...
	public function check_overdue_crons() {
		$overdue_crons = WP_DbOptimize()->howmany_overdue_crons();

		if ($overdue_crons >= 4) {
			return array('m' => WP_DbOptimize()->show_admin_warning_overdue_crons($overdue_crons));
		}
	}
	...
	public function enable_gzip_compression($params) {
		return WP_DbOptimize()->get_gzip_compression()->enable_gzip_command_handler($params);
	}
	...
	public function get_gzip_compression_status() {
		$status = WP_DbOptimize()->get_gzip_compression()->is_gzip_compression_enabled(true);
	...
	public function import_settings($params) {
		...
		return WP_DbOptimize()->get_options()->save_settings($settings);
	}
	...
	public function dismiss_install_or_update_notice() {
		if (!is_a(WP_DbOptimize()->install_or_update_notice, 'WP_DbOptimize_Install_Or_Update_Notice') || !is_callable(array(WP_DbOptimize()->install_or_update_notice, 'dismiss'))) {
		...
		if (!WP_DbOptimize()->install_or_update_notice->dismiss()) {
	...
	public function images_trash_command($params) {
		if (!class_exists('WP_DbOptimize_Images_Trash_Manager_Commands')) {
			return array(
				'errors' => array('WP_DbOptimize_Images_Trash_Manager_Commands class not found'),
			);
		}
		...
		$allowed_commands = WP_DbOptimize_Images_Trash_Manager_Commands::get_allowed_ajax_commands();
		...
		$results = call_user_func(array(WP_DbOptimize_Images_Trash_Manager()->commands, $trash_command), $params);
	...
	public function power_tweak($params) {
		...
		if (!is_a($wp_optimize_premium, 'WP_DbOptimize_Premium') || !property_exists($wp_optimize_premium, 'power_tweaks') || !isset($params['sub_action'])) return array(
