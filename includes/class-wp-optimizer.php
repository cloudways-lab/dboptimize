...
class WP_DbOptimizer {
	...
	public function get_optimizations_list() {
		...
		return apply_filters('wp_dboptimize_get_optimizations_list', $optimizations);
	...
	public function sort_optimizations_traditional($a, $b, $sort_on = 'ui_sort_order') {
		...
		if (!is_a($a, 'WP_DbOptimization')) return (!is_a($b, 'WP_DbOptimization')) ? 0 : 1;
		if (!is_a($b, 'WP_DbOptimization')) return -1;
	...
	public function do_optimization($which_optimization) {
		...
		if (apply_filters('wp_dboptimize_do_optimization', true, $which_optimization, $optimization)) {
		...
		do_action('wp_dboptimize_after_optimization', $which_optimization, $optimization);
	...
	public function get_table_prefix($allow_override = false) {
		...
		return ($allow_override) ? apply_filters('wp_dboptimize_get_table_prefix', $prefix) : $prefix;
	}
	...
	public function get_tables($update = false) {
		...
		$wpo_db_info = WP_DbOptimize()->get_db_info();

		$table_status = WP_DbOptimize()->get_db_info()->get_show_table_status($update);
		...
		if (is_array($table_status)) {
			...
			foreach ($table_status as $index => $table) {
				...
				$include_table = apply_filters('wp_dboptimize_get_tables_include_table', $include_table, $table_name, $table_prefix);
			...
			WP_DbOptimize()->get_options()->update_option('corrupted-tables-count', $corrupted_tables_count);
		}

		$tables_info = apply_filters('wp_dboptimize_get_tables', $table_status);
		return $tables_info;
	}
	...
	public function get_table($table_name) {
		...
		$db_info = WP_DbOptimize()->get_db_info();
		...
		$table = apply_filters('wp_dboptimize_get_table', $table);
		return $table;
	}
	...
	public function join_plugin_information($table_name, $table_obj) {
		...
		$table_obj->is_using = WP_DbOptimize()->get_db_info()->is_table_using_by_plugin($table_name);
		// if table belongs to any plugin then add plugins status.
		$plugins = WP_DbOptimize()->get_db_info()->get_table_plugin($table_name);
		...
		if (false !== $plugins) {
			...
			foreach ($plugins as $plugin) {
				$status = WP_DbOptimize()->get_db_info()->get_plugin_status($plugin);
	...
	public function enable_linkbacks($type, $enable = true) {
		...
		$wpdb = $GLOBALS['wpdb'];
		$wpo_options = WP_DbOptimize()->get_options();
	...
	public function trackback_comment_actions($options) {
		...
		if (isset($options['comments'])) {
			if (!$options['comments']) {
				...
				$messages[] =  sprintf(__('All comments on existing posts were disabled at %s.', 'wp-optimize'), WP_DbOptimize()->format_date_time(time()));
			} else {
				...
				$messages[] =  sprintf(__('All comments on existing posts were enabled at %s.', 'wp-optimize'), WP_DbOptimize()->format_date_time(time()));
			}
		}
		...
		if (isset($options['trackbacks'])) {
			if (!$options['trackbacks']) {
				...
				$messages[] =  sprintf(__('All trackbacks on existing posts were disabled at %s.', 'wp-optimize'), WP_DbOptimize()->format_date_time(time()));
			} else {
				...
				$messages[] =  sprintf(__('All trackbacks on existing posts were enabled at %s.', 'wp-optimize'), WP_DbOptimize()->format_date_time(time()));
			}
		}
