...
if (!class_exists('Updraft_Task_Manager_1_2')) require_once(WPODB_PLUGIN_MAIN_PATH . 'vendor/team-updraft/common-libs/src/updraft-tasks/class-updraft-task-manager.php');

if (!class_exists('WP_DbOptimize_Images_Trash_Manager')) :

class WP_DbOptimize_Images_Trash_Manager extends Updraft_Task_Manager_1_2 {
	...
	public function __construct() {
		...
		if (!class_exists('WP_DbOptimize_Images_Trash_Manager_Commands')) include_once('class-wp-optimize-images-trash-manager-commands.php');

		$this->commands = new WP_DbOptimize_Images_Trash_Manager_Commands($this);
	...
	private function save_to_cache($key, $value, $blog_id) {
		...
		WP_DbOptimize_Transients_Cache::get_instance()->set_transient($key, $value);
	}
	...
	private function get_from_cache($key, $blog_id) {
		...
		return WP_DbOptimize_Transients_Cache::get_instance()->get_transient($key);
	}
	...
	public function clear_cached_data($blog_id) {
		...
		foreach ($cached_keys as $key) {
			WP_DbOptimize_Transients_Cache::get_instance()->delete_transient($key);
		}
...
/**
 * Returns a WP_DbOptimize_Images_Trash_Manager instance
 */
function WP_DbOptimize_Images_Trash_Manager() {
	return WP_DbOptimize_Images_Trash_Manager::instance();
}
