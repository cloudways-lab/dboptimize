...
if (!class_exists('Updraft_Abstract_Logger')) require_once(WPODB_PLUGIN_MAIN_PATH.'/includes/class-updraft-abstract-logger.php');
if (!class_exists('Updraft_PHP_Logger')) require_once(WPODB_PLUGIN_MAIN_PATH.'/includes/class-updraft-php-logger.php');

/**
 * Class WP_DbOptimize_Tasks_Queue
 */
class WP_DbOptimize_Tasks_Queue {
	...
	/**
	 * WP_DbOptimize_Tasks_Queue constructor.
	 *
	...
	public function __construct($queue_id) {
		...
		$this->_lock = new WP_DbOptimize_Semaphore($queue_id);
	...
	public function lock() {
		$this->log('WP_DbOptimize_Tasks_Queue->lock()');
	...
	public function unlock() {
		$this->log('WP_DbOptimize_Tasks_Queue->unlock()');
		$this->_lock->unlock();
		$this->_locked = null;
	}
	...
	public function wait() {
		...
		$this->log('WP_DbOptimize_Tasks_Queue->wait()');
	...
	/**
	 * Return instance of WP_DbOptimize_Tasks_Queue.
	 *
	 * @param  string $queue_id
	 * @return object WP_DbOptimize_Tasks_Queue
	 */
	public static function this($queue_id = 'queue') {
		if (!array_key_exists($queue_id, self::$_instances)) {
			self::$_instances[$queue_id] = new WP_DbOptimize_Tasks_Queue($queue_id);
	...
	public function delete_queue() {
		...
		$this->log('WP_DbOptimize_Tasks_Queue->delete_queue()');
	...
	public function clear_queue() {
		$this->log('WP_DbOptimize_Tasks_Queue->clear_queue()');
	 ...
	 * Add task to queue.
	 *
	 * @param WP_DbOptimize_Queue_Task $task
	 */
	public function add_task(WP_DbOptimize_Queue_Task $task) {
		$this->log('WP_DbOptimize_Tasks_Queue->add_task({task})', array('task' => $task->get_info()));
	...
	public function load() {
		...
		$this->log('WP_DbOptimize_Tasks_Queue->load()');
		...
		foreach ($_tasks as $task_id => $info) {
			...
			$task = new WP_DbOptimize_Queue_Task($info['task'], $info['params'], $info['callback'], $info['priority']);
			$task->set_id($task_id);
			$task->set_queue_id($this->_queue_id);
			$this->_queue[] = $task;
		}
	 ...
	 *
	 * @param WP_DbOptimize_Queue_Task $task_a
	 * @param WP_DbOptimize_Queue_Task $task_b
	 * @return int
	 */
	public function cmp_order(WP_DbOptimize_Queue_Task $task_a, WP_DbOptimize_Queue_Task $task_b) {
	...
	public function flush() {
		...
		$this->log('WP_DbOptimize_Tasks_Queue->flush()');
	 ...
	 *
	 * @return bool|WP_DbOptimize_Queue_Task
	 */
	public function get_next_task() {
		// global $wpdb;

		$this->log('WP_DbOptimize_Tasks_Queue->get_next_task()');
	...
	public function do_next_task() {
		$this->log('WP_DbOptimize_Tasks_Queue->do_next_task()');
		...
		// do task.
		if (is_object($task) && is_a($task, 'WP_DbOptimize_Queue_Task')) {

			$this->log('WP_DbOptimize_Tasks_Queue->run({task})', array('task' => $task->get_info()));
			$task->run();
