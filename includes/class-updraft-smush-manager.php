...
if (!class_exists('Updraft_Task_Manager_1_2')) require_once(__DIR__ . '/../vendor/team-updraft/common-libs/src/updraft-tasks/class-updraft-task-manager.php');
...
class Updraft_Smush_Manager extends Updraft_Task_Manager_1_2 {
	...
	public function __construct() {
		...
		if (!class_exists('WP_DbOptimize_Transients_Cache')) include_once('class-wp-optimize-transients-cache.php');
	...
	public function manage_media_custom_column($column, $attachment_id) {
		...
		if (0 == $original_size) {
			$info = sprintf(__('The file was compressed to %s using WP-Optimize', 'wp-optimize'), WP_DbOptimize()->format_size($smushed_size));
		} else {
			$saved = round((($original_size - $smushed_size) / $original_size * 100), 2);
			$info = sprintf(__('The file was compressed from %s to %s, saving %s percent, using WP-Optimize', 'wp-optimize'), WP_DbOptimize()->format_size($original_size), WP_DbOptimize()->format_size($smushed_size), $saved);
		}
	...
	public function render_smush_metabox($post) {
		...
		WP_DbOptimize()->include_template('admin-metabox-smush.php', false, $extract);
	}
	...
	private function is_image_compressed_by_another_plugin($image_id) {
		...
		if (WP_DbOptimize()->get_db_info()->table_exists('ewwwio_images')) {
	...
	public function ewww_image_optimizer_compressed_images_args($args) {
		...
		if (!WP_DbOptimize()->get_db_info()->table_exists('ewwwio_images')) return $args;
	...
	public function is_multisite_mode() {
		return WP_DbOptimize()->is_multisite_mode();
	}
	...
	/**
	 * Instance of WP_DbOptimize_Page_Cache_Preloader.
	 *
	 * @return self
	 */
