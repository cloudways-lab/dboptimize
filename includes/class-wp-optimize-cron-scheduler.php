...
if (!class_exists('WP_DbOptimize_Cron_Scheduler')) {

class WP_DbOptimize_Cron_Scheduler {

	/**
	 * Holds an instance of semaphore lock class
	 *
	 * @var WP_DbOptimize_Semaphore $_lock
	 */
	 ...
	 *
	 * @return null|WP_DbOptimize_Cron_Scheduler Singleton Instance
	 */
	...
	public function cron_action($cron_id, $schedule) {
		...
		WP_DbOptimize()->log('WPO: Starting cron_action()');
				...
				if ('optimizetables' === $optimization_id) {
					...
					if (false === $tablesstatus['is_optimizable'] && $tablesstatus['inno_db_tables'] > 0) {
						...
						if ($force_optimization) {
							WP_DbOptimize()->log('WPO: Chooses to optimize InnoDB tables');
						} else {
							WP_DbOptimize()->log('WPO: Chooses to not optimize InnoDB tables');
						}
	...
	public function cron_activate() {
		...
		foreach ($auto_options as $event) {
			...
			WP_DbOptimize()->log($event['schedule_type']);
	...
	private function set_daily_cron($event, $cron_id) {
		...
		WP_DbOptimize()->log('wpo_cron_event3 - ' . $cron_id . ' - ' . $selected_schedule);
	}
	...
	private function set_once_cron($event, $cron_id) {
		...
		WP_DbOptimize()->log('wpo_cron_event3 - once');
	}
	...
	public function set_semaphore_lock($semaphore = 'wpo') {
		...
		WP_DbOptimize()->log($semaphore_log_message);
		if (!self::$_lock->lock()) {
			WP_DbOptimize()->log('Failed to gain semaphore lock ('.$semaphore.') - another optimization of this type is apparently already active - aborting');
			return false;
		}
		return true;
	}
	...
	public function wpo_cron_deactivate() {
		$auto_options = WP_DbOptimize()->get_options()->get_option('auto');
