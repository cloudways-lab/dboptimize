...
if (!class_exists('WP_DbOptimize_Updates')) :

class WP_DbOptimize_Updates {
	...
	public static function enable_minify_defer() {
		if (!function_exists('wp_dboptimize_minify_config')) {
			include_once WPODB_PLUGIN_MAIN_PATH . '/minify/class-wp-optimize-minify-config.php';
		}
		$current_setting = wp_dboptimize_minify_config()->get('enable_defer_js');
		if (true === $current_setting) {
			wp_dboptimize_minify_config()->update(array('enable_defer_js' => 'all'));
		} else {
			wp_dboptimize_minify_config()->update(array('enable_defer_js' => 'individual'));
		}
	}
	...
	public static function update_minify_excludes() {
		...
		if (!class_exists('WP_DbOptimize_Minify_Functions')) {
			include_once WPODB_PLUGIN_MAIN_PATH . '/minify/class-wp-optimize-minify-functions.php';
		}
		...
		$current_blacklist = wp_dboptimize_minify_config()->get('blacklist');
		$current_ignorelist = wp_dboptimize_minify_config()->get('ignore_list');
		...
		$default_blacklist = WP_DbOptimize_Minify_Functions::get_default_ie_blacklist();
		$default_ignorelist = WP_DbOptimize_Minify_Functions::get_default_ignore();
		...
		// Update the options
		wp_dboptimize_minify_config()->update(array('blacklist' => $user_excluded_blacklist_items));
		wp_dboptimize_minify_config()->update(array('ignore_list' => $user_excluded_ignorelist_items));
	}
}
