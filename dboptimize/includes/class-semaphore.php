...
class WP_DbOptimize_Semaphore {
	...
	public function __construct($semaphore = 'lock') {
		...
		if (!is_array($results) || count($results) < 3) {
			...
			if (is_array($results) && count($results) > 0) {
				WP_DbOptimize()->log("Semaphore ($semaphore, ".$wpdb->options.") in an impossible/broken state - fixing (".count($results).")");
			} else {
				WP_DbOptimize()->log("Semaphore ($semaphore, ".$wpdb->options.") being initialised");
			}
	...
	public function lock() {
		...
		if ('0' == $affected && !$this->stuck_check()) {
			WP_DbOptimize()->log('Semaphore lock (' . $this->lock_name . ', ' . $wpdb->options . ') failed (line ' . __LINE__ . ')');
			return false;
		}
		...
		if ('1' != $affected) {
			...
			WP_DbOptimize()->log('Semaphore (' . $this->lock_name . ', ' . $wpdb->options . ') reset to 1');
		}
		...
		", current_time('mysql', 1)));
		WP_DbOptimize()->log('Set semaphore last lock (' . $this->lock_name . ') time to ' . current_time('mysql', 1));

		WP_DbOptimize()->log('Semaphore lock (' . $this->lock_name . ') complete');
		return true;
	}
		...
		if (count($filters)) {
			...
			$wpdb->query("
				UPDATE $wpdb->options
				 ...
				 WHERE option_name = 'wpo_semaphore_".$this->lock_name."'
			");
			WP_DbOptimize()->log('Incremented the semaphore ('.$this->lock_name.') by 1');
		}
	...
	public function decrement() {
		...
		");
		WP_DbOptimize()->log('Decremented the semaphore (' . $this->lock_name . ') by 1');
	}
	...
	public function unlock() {
		...
		if ('1' == $result) {
			WP_DbOptimize()->log('Semaphore (' . $this->lock_name . ') unlocked');
			return true;
		}

		WP_DbOptimize()->log('Semaphore (' . $this->lock_name . ', ' . $wpdb->options . ') still locked (' . $result . ')');
		return false;
	}
	...
	private function stuck_check() {
		...
		if ('1' == $affected) {
			WP_DbOptimize()->log('Semaphore (' . $this->lock_name . ', ' . $wpdb->options . ') was stuck, set lock time to ' . $current_time);
