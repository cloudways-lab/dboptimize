...
/**
 * Class WP_DbOptimize_Gzip_Compression
 */
class WP_DbOptimize_Gzip_Compression {

	/**
	 * WP_DbOptimize_Htaccess instance.
	 *
	 * @var WP_DbOptimize_Htaccess
	 */
	private $_htaccess = null;

	/**
	 * WP_DbOptimize instance.
	 *
	 * @var WP_DbOptimize
	 */
	...
	/**
	 * WP_DbOptimize_Gzip_Compression constructor.
	 */
	public function __construct() {
		$this->_wp_optimize = WP_DbOptimize();
	...
	public function get_headers_information() {
		...
		$headers = WP_DbOptimize()->get_stylesheet_headers();
	...
	public function is_gzip_compression_enabled($force_check = false) {
		...
		if (!$force_check) return WP_DbOptimize()->get_options()->get_option('is_gzip_compression_enabled');
		...
		if (false == $is_gzip_compression_enabled && $this->is_gzip_compression_section_exists()) {
			...
			} elseif (WP_DbOptimize()->is_apache_server()) {
		...
		WP_DbOptimize()->get_options()->update_option('is_gzip_compression_enabled', $is_gzip_compression_enabled);
	...
	public function restore() {
		$enabled = WP_DbOptimize()->get_options()->get_option('is_gzip_compression_enabled');

		if ($enabled && $this->_htaccess->is_writable()) $this->enable();
	}

	/**
	 * Handler for Gzip compression enable command, called from WP_DbOptimize_Commands.
	 *
