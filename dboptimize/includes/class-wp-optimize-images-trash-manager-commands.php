...
if (!class_exists('Updraft_Task_Manager_Commands_1_0')) require_once(WPODB_PLUGIN_MAIN_PATH . 'vendor/team-updraft/common-libs/src/updraft-tasks/class-updraft-task-manager-commands.php');

if (!class_exists('WP_DbOptimize_Images_Trash_Manager_Commands')) :

class WP_DbOptimize_Images_Trash_Manager_Commands extends Updraft_Task_Manager_Commands_1_0 {
	...
	public function move_images_to_trash($data) {
		...
		foreach ($data['images'] as $image) {
			...
			WP_DbOptimize_Images_Trash_Task::create_task('move_image_to_trash', '', $options);
		}
		...
		$success_count = WP_DbOptimize_Images_Trash_Manager::instance()->get_stat_value('move_image_to_trash_success', 0);
		$total_count = $success_count + WP_DbOptimize_Images_Trash_Manager::instance()->get_stat_value('move_image_to_trash_failure', 0);

		$moved_images = WP_DbOptimize_Images_Trash_Manager::instance()->get_stat_value('moved_images', array());
		WP_DbOptimization_images::instance()->delete_selected_images_from_cache($moved_images);
	...
	public function get_trash_images($data) {
		...
		$base_url = WP_DbOptimize_Images_Trash_Manager::instance()->get_trash_url();
		$images = WP_DbOptimize_Images_Trash_Manager::instance()->get_trash_images($length, $offset, $blog_id);
	...
	public function restore_images_from_trash($data) {
		...
		foreach ($data['images'] as $image) {
			...
			// add task to queue for processing images
			WP_DbOptimize_Images_Trash_Task::create_task('restore_image_from_trash', '', array(
				'file' => $image,
			));
		}
		...
		$success_count = WP_DbOptimize_Images_Trash_Manager::instance()->get_stat_value('restore_image_from_trash_success', 0);
		$total_count = $success_count + WP_DbOptimize_Images_Trash_Manager::instance()->get_stat_value('restore_image_from_trash_failure', 0);

		$restored_images = WP_DbOptimize_Images_Trash_Manager::instance()->get_stat_value('restored_images', array());
		WP_DbOptimization_images::instance()->add_selected_images_to_cache($restored_images);
	...
	public function remove_trash_images($data) {
		...
		foreach ($data['images'] as $image) {
			...
			// add task to queue for processing images
			WP_DbOptimize_Images_Trash_Task::create_task('remove_trash_images', '', array(
				'file' => $image,
			));
		}
		...
		$success_count = WP_DbOptimize_Images_Trash_Manager::instance()->get_stat_value('remove_trash_images_success', 0);
		$total_count = $success_count + WP_DbOptimize_Images_Trash_Manager::instance()->get_stat_value('remove_trash_images_failure', 0);
