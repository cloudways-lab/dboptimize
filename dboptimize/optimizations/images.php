...
if (!class_exists('Updraft_Abstract_Logger')) require_once(__DIR__.'/../includes/class-updraft-abstract-logger.php');
if (!class_exists('Updraft_PHP_Logger')) require_once(__DIR__.'/../includes/class-updraft-php-logger.php');
if (!class_exists('WP_DbOptimization_Images_Shutdown')) require_once(__DIR__.'/../includes/class-wp-optimization-images-shutdown.php');

/**
 * Class WP_DbOptimization_images
 */
class WP_DbOptimization_images extends WP_DbOptimization {
	...
	public function __construct($data = array()) {
		...
		if ($this->is_multisite_mode()) {
			$_sites = WP_DbOptimize()->get_sites();
	...
	/**
	 * Get WP_DbOptimization_images instance.
	 *
	 * @return WP_DbOptimization_images
	 */
	public static function instance() {
		if (!self::$instance) {
			self::$instance = new WP_DbOptimization_images();
		}
	...
	/**
	 * Returns WP_DbOptimize_Tasks_Queue instance.
	 *
	 * @return WP_DbOptimize_Tasks_Queue
	 */
	private function _tasks_queue() {
		return WP_DbOptimize_Tasks_Queue::this($this->get_work_mode());
	}
	...
	public function after_optimize() {
		...
		// flush cached values.
		WP_DbOptimize_Transients_Cache::get_instance()->flush();
	}
	...
	private function build_get_info_output($params = array(), $removed = null, $output_removed_message = false) {
		...
		if ($return_images) {
			...
			foreach (array_keys($unused_images) as $blog_id) {
				...
				$images_loaded_text[$blog_id] = sprintf(__('%s of %s images loaded', 'wp-dboptimize'), $images_loaded, $total_images_count);
			}
		...
		if (null !== $removed) {
			...
			$message = sprintf(_n('%s unused image removed with a total size of ', '%s unused images removed with a total size of ', $total_files, 'wp-dboptimize') . $this->size_format($total_size), number_format_i18n($total_files), 'wp-dboptimize');
		...
		if ($total_files > 0) {
			if (null !== $removed && $output_removed_message) {
				...
				$message = sprintf(_n('%s unused image found with a total size of ', '%s unused images found with a total size of ', $total_files, 'wp-dboptimize') . $this->size_format($total_size), number_format_i18n($total_files), 'wp-dboptimize');
			}
		} else {
			$message = __('No unused images found', 'wp-dboptimize');
		}
	...
	public function before_get_info() {
		...
		if (isset($this->data['cancel'])) {
			...
			WP_DbOptimization_Images_Shutdown::get_instance()->reset_values();
			return;
		}
	...
	public function get_info() {
		...
		if ($this->_tasks_queue()->get_meta('new_queue')) {
			$this->_tasks_queue()->add_task(new WP_DbOptimize_Queue_Task(array(get_class($this), 'task_get_info'), array($blog_id), '', $this->calc_priority($blog_id, 1)));
		}
	}
	...
	public function after_get_info() {
		...
		WP_DbOptimization_Images_Shutdown::get_instance()->activate();
		WP_DbOptimization_Images_Shutdown::get_instance()->set_meta($this->get_meta());
		// Save queue id for unlock if fatal error happens.
		WP_DbOptimization_Images_Shutdown::get_instance()->set_value('queue_id', $this->get_work_mode());
		...
		WP_DbOptimization_Images_Shutdown::get_instance()->deactivate();
		// flush cached values.
		WP_DbOptimize_Transients_Cache::get_instance()->flush();
		// flush queue.
	...
	public function task_get_info($blog_id) {
		...
		$this->message(__('Getting information...', 'wp-dboptimize'), $blog_id);
		...
		if (self::DETECT_BOTH == $mode || self::DETECT_IMAGES == $mode) {
			...
			if (!is_array($unused_posts_images)) {
				$this->_tasks_queue()->add_task(new WP_DbOptimize_Queue_Task(array(get_class($this), 'task_get_posts_images'), array(0, $this->_posts_per_request, $blog_id), array(get_class($this), 'process_get_posts_images_result'), $this->calc_priority($blog_id, 5)));
			}

			// check if upload directory images already in the cache.
			$unused_images_files = $this->get_from_cache('unused_images_files', $blog_id);
			if (!is_array($unused_images_files)) {
				$this->_tasks_queue()->add_task(new WP_DbOptimize_Queue_Task(array(get_class($this), 'task_get_unused_images_files'), array($blog_id), '', $this->calc_priority($blog_id, 10)));
			}
		}
		...
		if (self::DETECT_BOTH == $mode || self::DETECT_SIZES == $mode) {
			...
			$all_image_sizes = $this->get_from_cache('all_image_sizes', $blog_id);
			if (!is_array($all_image_sizes)) {
				$this->_tasks_queue()->add_task(new WP_DbOptimize_Queue_Task(array(get_class($this), 'task_get_all_image_sizes'), array(0, 1000, $blog_id), array(get_class($this), 'process_get_all_image_sizes_results'), $this->calc_priority($blog_id, 15)));
			}
		}
	}
	...
	private function get_posts_content_images(&$posts, $blog_id) {
		...
		WP_DbOptimization_Images_Shutdown::get_instance()->set_value('blog_id', $blog_id);
		...
		foreach ($posts as $post) {
			...
			WP_DbOptimization_Images_Shutdown::get_instance()->set_value('last_post_id', $post->ID);

			// if post in "bad posts" list then we don't use do_shortcode.
			if (WP_DbOptimization_Images_Shutdown::get_instance()->is_bad_post($blog_id, $post->ID)) {
			...
			WP_DbOptimization_Images_Shutdown::get_instance()->delete_value('last_post_id');
	...
	public function process_get_posts_images_result($result) {
		...
		if ($result['processed'] < $result['limit']) {
			...
			$this->message(__('Posts checked.', 'wp-dboptimize'), $blog_id);
			...
			$this->_tasks_queue()->add_task(new WP_DbOptimize_Queue_Task(array(get_class($this), 'task_get_posts_images'), array($new_offset, $result['limit'], $blog_id), array(get_class($this), 'process_get_posts_images_result'), $this->calc_priority($blog_id, 5)));

			$this->message(sprintf(_n('%s post processed...', '%s posts processed...', $new_offset, 'wp-dboptimize'), $new_offset), $blog_id);
		}
	}
	...
	public function task_get_unused_images_files($blog_id = 1) {
		...
		$this->message(__('Checking upload directory...', 'wp-dboptimize'), $blog_id);

		$sub_dirs = apply_filters('wpo_unused_images_sub_dirs', $this->get_upload_sub_dirs($blog_id), $blog_id, $this);

		if (!empty($sub_dirs)) {
			foreach ($sub_dirs as $sub_dir) {
				$this->_tasks_queue()->add_task(new WP_DbOptimize_Queue_Task(array(get_class($this), 'get_orphaned_images_in_sub_directory'), array($sub_dir, $blog_id), array(get_class($this), 'process_get_orphaned_images_in_sub_directory'), $this->calc_priority($blog_id, 11)));
			}
		}

		$this->_tasks_queue()->add_task(new WP_DbOptimize_Queue_Task(array(get_class($this), 'process_get_unused_images_files_result'), array($blog_id), '', $this->calc_priority($blog_id, 12)));
	}
	...
	public function process_get_unused_images_files_result($blog_id) {
		...
		$this->message(__('Process results...', 'wp-dboptimize'), $blog_id);
	...
	public function get_orphaned_images_in_sub_directory($sub_directory, $blog_id = 1) {
		...
		$this->message(__('Checking upload directory...', 'wp-dboptimize').' ['.$sub_directory.']', $blog_id);
	...
	public function task_get_all_image_sizes($offset = 0, $limit = 1000, $blog_id = 1) {
		...
		$this->message(__('Get information about image sizes...', 'wp-dboptimize'), $blog_id);
	...
	public function process_get_all_image_sizes_results($result) {
		...
		if (count($result['image_ids']) == $result['limit']) {
			...
			$this->_tasks_queue()->add_task(new WP_DbOptimize_Queue_Task(array(get_class($this), 'task_get_all_image_sizes'), array($new_offset, $result['limit'], $this->calc_priority($blog_id, 15))));
		} else {
	...
	public function settings_label() {
		return __('Remove unused images', 'wp-dboptimize');
	}
	...
	private function save_to_cache($key, $value, $blog_id = 1) {
		...
		return WP_DbOptimize_Transients_Cache::get_instance()->set($key, $value, $transient_limit);
	}
	...
	private function get_from_cache($key, $blog_id = 1) {
		...
		$value = WP_DbOptimize_Transients_Cache::get_instance()->get($key);
	...
	public function delete_selected_images_from_cache($images) {
		...
		foreach (array_keys($_images) as $blog_id) {
			...
			$unused_posts_images = $update_images ? WP_DbOptimization_images::instance()->get_from_cache('unused_posts_images', $blog_id) : array();
			$unused_images_files = $update_files ? WP_DbOptimization_images::instance()->get_from_cache('unused_images_files', $blog_id) : array();
			...
			if ($update_images) WP_DbOptimization_images::instance()->save_to_cache('unused_posts_images', $unused_posts_images, $blog_id);
			if ($update_files) WP_DbOptimization_images::instance()->save_to_cache('unused_images_files', $unused_images_files, $blog_id);
	...
	private function delete_from_cache($key, $blog_id = 1) {
		...
		WP_DbOptimize_Transients_Cache::get_instance()->delete($key);
	...
	private function preload_attachments_metadata(&$attachment_ids) {
		...
		$preload_batch_limit = floor(WP_DbOptimize()->get_free_memory() / $item_size);
	...
	private function is_debug_mode() {
		return (defined('WP_DBOPTIMIZE_DEBUG_OPTIMIZATIONS') && WP_DBOPTIMIZE_DEBUG_OPTIMIZATIONS);
	}
	...
	private function log($message, $context = array()) {
		...
		if (defined('WP_DBOPTIMIZE_UNUSED_IMAGES_LOG') && WP_DBOPTIMIZE_UNUSED_IMAGES_LOG) {
