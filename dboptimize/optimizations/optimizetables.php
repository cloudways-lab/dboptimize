...
class WP_DbOptimization_optimizetables extends WP_DbOptimization {
	...
	private function optimize_table($table_obj, $force = false) {
		...
		if ($table_obj->is_type_supported) {
			...
			$this->register_output(__('Optimized table:', 'wp-dboptimize') . ' ' . $table_obj->Name);
		}
	}
	...
	public function after_get_info() {
		...
		if (false === $tablesstatus['is_optimizable']) {
			if (isset($this->data['optimization_table']) && '' != $this->data['optimization_table']) {
				...
				$this->register_output(__('Total gain:', 'wp-dboptimize').' '.WP_DbOptimize()->format_size(($tablesstatus['total_gain'])));
			}
			...
			if ($tablesstatus['inno_db_tables'] > 0) {
				...
				if ($tablesstatus['non_inno_db_tables'] > 0) {
					$this->register_output(sprintf(__('Other tables will be optimized (%s).', 'wp-dboptimize'), $tablesstatus['non_inno_db_tables']));
				}
				...
				$this->register_output('<input id="innodb_force_optimize" name="innodb-force-optimize" type="checkbox" value="true" '.checked($force_db_option, 'true').'><label for="innodb_force_optimize">'.__('Optimize InnoDB tables anyway.', 'wp-dboptimize').'</label><br><a href="'.$faq_url.'" target="_blank">'.__('Warning: you should read the FAQ on the risks of this operation first.', 'wp-dboptimize').'</a>');
			}
		} else {
			$this->register_output(sprintf(__('Tables will be optimized (%s).', 'wp-dboptimize'), $tablesstatus['non_inno_db_tables'] + $tablesstatus['inno_db_tables']));
		}
	}

	public function get_auto_option_description() {
		return __('Optimize database tables', 'wp-dboptimize');
	}
